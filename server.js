...
mongoose.connect('mongodb://abeer:abeer@ds249545.mlab.com:49545/point' , {useMongoClient: true}); //mongodb://localhost:27017/pttt
...
app.get('/api' , function(req , res , next){
	...
	if(req.query.p === undefined){
		...
		qApi.sort({date: -1}).limit(12).then(function(subPptArray){
		...
		var x = 12*(req.query.p - 1);
		qApi.sort({date: -1}).skip(x).limit(12)
...
app.get('/apiSort' , function(req , res , next){
	...
	if(req.query.p === undefined){
		...
		var x = 12*(req.query.p - 1);
		qApiSort.sort(ssort).skip(x).limit(12)
		.then(function(subPptArray){
		   res.json({subPptArray});
		   next();
		});
...
app.get('/search' , function(req , res , next){
	...
	if(req.query.p === undefined){
		...
		q.sort(sssort).limit(12).then(function(subPptArray){
		   res.json({subPptArray});
		});
	}
	else{
		var x = 12*(req.query.p - 1);
		q.sort(sssort).skip(x).limit(12).then(function(subPptArray){
		   res.json({subPptArray});
		});
	}
...
var port = process.env.PORT || 4000; //port for heroku
